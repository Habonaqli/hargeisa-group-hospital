...
function cleanUser(user) {
  return {
    ...
    id: user._id instanceof ObjectId ? user._id.toString() : user._id,
...
async function handleLogin(req, res, db) {
  ...
  if (user.status !== "Active") {
    return res.status(403).json({
      message: "Your account is waiting for admin approval or has been disabled.",
    });
  }
...
async function handleUsers(req, res, db) {
  ...
  if (req.method === "PUT") {
    ...
    const updates = {
      ...
      email: email ? String(email).toLowerCase() : existingUser.email,
...
async function handleApproveUser(req, res, db) {
  if (req.method !== "POST") {
    return res.status(405).json({ message: "Method not allowed" });
  }
  ...
  if (action === "approve" && !allowedApprovalRoles.includes(role)) {
    return res.status(400).json({
...
export default async function handler(req, res) {
  try {
    ...
    if (type === "approve-user") {
      return await handleApproveUser(req, res, db);
    }

    return res.status(400).json({
      message:
        "Invalid auth type. Use ?type=login, ?type=register, ?type=users, or ?type=approve-user",
    });
  } catch (error) {
